function [fig,legend_str,return_period,sorted_damage] = barisal_EDS_DFC(EDS,EDS_comparison,Percentage_Of_Value_Flag,plot_loglog)
% climada
% NAME:
...
for EDS_i=1:length(EDS)
    ...
    % ---just for Barisal----
    sorted_damage = sorted_damage/10^9;
    if numel(return_period) == 4
        %return_period = 1./EDS(EDS_i).frequency([3 4 2 1]);
        return_period = [42 14 12.5 12];
    end %------------------
...
if Percentage_Of_Value_Flag
    ...
    ylabel('Damage (BDT billions)')% ---just for Barisal----
end
% set(gcf,'Color',[1 1 1])
