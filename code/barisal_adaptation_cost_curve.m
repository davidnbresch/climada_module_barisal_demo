function [insurance_benefit,insurance_cost]=barisal_adaptation_cost_curve(measures_impact, measures_impact_comparison,x_text_control,y_text_control,scaled_AED,nice_numbers,reverse_cb,plot_arrows)
...
if nice_numbers
    ...
    if nice_numbers == 1
        fct        = 10^-8;
        nr_format  = '\t %4.1f';
        xlabel_str = sprintf('Averted damage (BDT millions)');
    else
        fct        = 10^-nice_numbers;
        nr_format  = '\t %4.1f';
        xlabel_str = sprintf('Averted damage over %d years (10^%1.0f BDT)',n_years, nice_numbers);
    end
else
    fct        = 1;
    nr_format  = '%2.1e';
    xlabel_str = sprintf('Averted damage over %d years (BDT)',n_years);
end
...
if scaled_AED
    fprintf('\t \t    \t\t\t(Mio BDT)\t\t(Mio BDT)\t\t(BDT/BDT)\n');
elseif nice_numbers>1
    fprintf('\t \t\t    \t\t(10^%1.0f BDT)\t\t(10^%1.0f BDT)\t\t(BDT/BDT)\n', nice_numbers, nice_numbers);
else
    fprintf('\t \t    \t\t\t(BDT)\t\t\t(BDT)\t\t\t(BDT/BDT)\n');
end
...
if reverse_cb
    ylabel('Benefit/cost ratio (BDT/BDT)','fontsize',fontsize_+1)
else
    ylabel('Cost/benefit ratio (BDT/BDT)','fontsize',fontsize_+1)
end;
...
% tcr_str = sprintf('Total climate risk\n%.0f BDT',tot_climate_risk*fct);
% tcr_str = sprintf('Total climate risk\n BDT %.0f m',round(tot_climate_risk*fct/100000)/10);
tcr_str = sprintf('Total climate risk\n BDT %.1f m',round(tot_climate_risk*fct/100000)/10);
...
[tok,rem] = strtok(title_str,'|'); title_cl = {};
while ~isempty(tok)
    title_cl{end+1} = tok;
    title_cl{end+1} = '\otimes';
    [tok,rem] = strtok(rem,'|');
end
title_cl(end)= [];
% in case ~isempty(measures_impact_comparison)   
title_str=strrep(title_str,'|','\otimes'); % LaTEX format
title(title_cl,'FontSize',fontsize_);
